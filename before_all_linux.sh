...
./bootstrap.sh --with-libraries=date_time,filesystem,iostreams,log,regex,serialization,system,thread,timer,test
./b2 install -sZLIB_SOURCE=$CURRENT_DIR/zlib-1.3.1
cd ..
echo "XYZ END unpack boost"

echo "XYZ BEGIN unpack ORE"
# Build ORE
cd ore
mkdir build
cd build
export ZLIB_ROOT=$CURRENT_DIR/zlib-1.3.1
cmake -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DORE_USE_ZLIB=ON -DORE_BUILD_DOC=OFF -DORE_BUILD_SWIG=OFF -DORE_BUILD_EXAMPLES=OFF -DORE_BUILD_TESTS=OFF -DORE_BUILD_APP=OFF -DQL_BUILD_BENCHMARK=OFF -DQL_BUILD_EXAMPLES=OFF -DQL_BUILD_TEST_SUITE=OFF -DQL_ENABLE_SESSIONS=ON -DCMAKE_BUILD_TYPE=Release ..
cmake --build . -j4
cmake --install .
